...
def get_search_list(kw: str):
    ...
    post_url = "http://search.people.cn/search-platform/front/search"
    headers = {
        'User-Agent': random.choice(ua_list),
    ...
    contents = '{"key":"%s","page":%d,"limit":100,"hasTitle":true,"hasContent":true,"isFuzzy":false,"type":0,"sortType":0,"startTime":1641036156000,"endTime":1645701756000}'
...
def get_unloaded_raw_pages():
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT title, url FROM urls WHERE ISNULL(loaded) AND kw='冬奥' GROUP BY title ORDER BY ID")
        res = cur.fetchall()
    ...
    for item in res:
        ...
        if status == 403:
            intercept_cnt += 1
            if intercept_cnt >= 9:
                print('shit! get intercepted for 9 times. sleep 7 mins')
                for i in range(7):
                    sleep(60)
            continue
